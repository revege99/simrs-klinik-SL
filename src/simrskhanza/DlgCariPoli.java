...
public final class DlgCariPoli extends javax.swing.JDialog {
    ...
    public DlgCariPoli(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"Kode Unit","Nama Unit","Registrasi Baru","Registrasi Lama", "Poli Sakit"};
    ...
    */
    ...
    private void tampil() {
        ...
        try {
            ...
            ps=koneksi.prepareStatement("SELECT p.*,mp.polisakit\n" +
                                            "FROM poliklinik p\n" +
                                            "LEFT JOIN maping_poliklinik_pcare mp ON p.kd_poli = mp.kd_poli_rs\n" +
                                            "WHERE p.status='1'");
            try{           
                rs=ps.executeQuery();
                while(rs.next()){
                    tabMode.addRow(new Object[]{rs.getString(1),rs.getString(2),Valid.SetAngka(rs.getDouble(3)),Valid.SetAngka(rs.getDouble(4))});
                    iyembuilder.append("{\"KodeUnit\":\"").append(rs.getString(1)).append("\",\"NamaUnit\":\"").append(rs.getString(2)).append("\",\"RegistrasiBaru\":\"").append(rs.getString(3)).append("\",\"RegistrasiLama\":\"").append(rs.getString(4)).append("\",\"polisakit\":\"").append(rs.getString(6)).append("\"},");
                }
            }catch(Exception e){
                ...
                if(TCari.getText().trim().equals("")){
                    for(JsonNode list:response){
                        tabMode.addRow(new Object[]{
                            list.path("KodeUnit").asText(),list.path("NamaUnit").asText(),list.path("RegistrasiBaru").asText(),list.path("RegistrasiLama").asText(),list.path("polisakit").asText()
                        });
                    }
                }else{
                    for(JsonNode list:response){
